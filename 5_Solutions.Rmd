...
1. Use the gap dataset to figure out a way to use `geom_line()` to create a line plot of average lifeExp through time with a separate line for each continent. Before plotting however, use dplyr to pipe in your `group_by()` and `summarize()` functions directly into ggplot. 
```{r}
library(dplyr)
library(ggplot2)
...
# Challenge 7 - conceptual p-value 0.08?

Click to view [du Prel JB. 2009. Confidence interval or p-value?](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2689604/)
